...
# Rownd Implementation Best Practices
...
When deploying Rownd in mobile applications:

1. **Implement Dual Token Validation**
```javascript
function validateAuthToken(token) {
    ...
    if (decoded.issuer === 'rownd') {
        return rowndSDK.validateToken(token);
    } else {
        return legacyAuth.validateToken(token);
    }
}
```
...
You can organize your applications hierarchically using sub-apps or sub-brands:
