...
| Constant | Description | Value | Usage |
|----------|-------------|--------|-------|
| `ROWND_TOKEN_CALLBACK_PATH` | Path for handling Rownd token callbacks | `/api/rownd-token-callback` | Used in middleware matcher config |
...
export const config = {
  matcher: [
    ROWND_TOKEN_CALLBACK_PATH,  // Required for token handling
...
For more examples and detailed client-side features, refer to the [React SDK documentation](/sdk-reference/web/react---next-js#examples).
